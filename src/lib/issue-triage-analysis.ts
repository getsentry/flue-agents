...
export const rewriteModeSchema = v.picklist([
  "none",
  "light_cleanup",
  "technical_diagnosis",
  "scope_clarification",
...
export const commentKindSchema = v.picklist([
  "none",
...
export const issueTriageDiagnosisSchema = v.object({
...
export function assertDiagnosisAnalysis(diagnosis: IssueTriageDiagnosis) {
  ...
  if (diagnosis.should_update_issue && !diagnosis.proposed_body?.trim()) {
    throw new Error("Issue updates require proposed_body.");
  }

  if (diagnosis.should_comment && !diagnosis.triage_comment?.trim()) {
    throw new Error("Triage comments require triage_comment.");
  }
